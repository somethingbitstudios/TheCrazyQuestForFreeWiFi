...
public class Gradual_movementRL : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (LeftWall == true && IsGrounded == false && IsJumping == false && IsDisting == false && IsWallJumping == false)
        {
        ...
        if (IsWallJumping == true)
        {
            ...
            if (rb.velocity.x < 15 && rb.velocity.x > -15 && WallJumpTime / 2 > 0.1 )
            {
                WallJump += Speed / 15 ;
            }
            //if (rb.velocity.x > -15 && WallJumpTime / 2 > 0.1)
            //{
            //    WallJump += Speed / 20 ;
            //}

            if (Input.GetKey(KeyCode.Space) && IsWallJumping == true)
            {
                ...
                rb.velocity = new Vector2(WallJump + Speed * ActualBoost, rb.velocity.y);
